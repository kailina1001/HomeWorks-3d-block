...
//!!Доп. Решение
/* const genres = movies.reduce((arr, { genre }) => {
...
//!!Доп. Решение
/* const actors = users.reduce((arr, { actors }) => {
...
const someMass = arr.map(({ id, title, released, plot }) => ({
  id,
  title,
  released,
  plot,
...
//!!Доп. Решение
/* const actorCareer = movies.reduce(
	(accum, film) =>
	  film.actors.reduce((acc, actor) => {
		if (!acc[actor]) {
		  return { ...acc, [actor]: [film] }
...
//!!Доп. Решение
/* const writers = [...new Set(movies.map((u) => u.writer))]; */
...
/* const getMoviesBySubstring = (films, substring) =>
  films.filter(({title}) => title.toLowerCase().includes(substring.toLowerCase()); */
...
//!!Доп. Решение
/* const getMoviesByYear = (films, year) => films.filter(({year}) => year); */
...
function task9(arr, str) {
  const titlePlot = arr.filter(
    (item) => item.title.includes(str) || item.plot.includes(str)
  );
  return titlePlot;
}
...
console.log(getFilterMoviesByField); */
